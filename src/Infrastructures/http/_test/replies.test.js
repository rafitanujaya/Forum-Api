...
describe('/threads/{threadId}/comments/{commentId}/replies', () => {
  let accessToken;
  let userId;
  const threadId = 'thread-432';
  const commentId = 'comment-432';
  ...
  describe('when DELETE /replies/{replyId}', () => {
    it('should response 200 with status success', async () => {
      ...
      await CommentsTableTestHelper.addComment({ id: 'comment-321', owner: userId, thread: threadId });
      await RepliesTableTestHelper.addReply({ id: 'reply-123', comment: 'comment-321', owner: userId });
      const server = await createServer(container);
